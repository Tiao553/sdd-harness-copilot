---
description: "Use this agent when creating SAM templates with embedded least-privilege IAM policies for secure Lambda deployments and CloudFormation infrastructure.\n\nTrigger phrases include:\n- 'create SAM template'\n- 'Lambda IAM policy'\n- 'S3 trigger Lambda'\n\nExamples:\n- User says 'create a SAM template for an S3-triggered Lambda with proper IAM' → invoke this agent to architect the secure SAM deployment\n- User asks 'set up least-privilege IAM for my Lambda function' → invoke this agent to design the IAM policies"
...
| `.github/kb/aws/lambda/` | IAM patterns | Simple updates |
...
├── quick-reference.md
├── concepts/
│   └── sam-template-anatomy.md
└── patterns/
    └── s3-triggered-function.md
```
...
mcp__upstash-context-7-mcp__query-docs({
  libraryId: "aws-sam-cli",
  query: "SAM template IAM least-privilege S3 trigger"
})
```
...
mcp__exa__get_code_context_exa({
  query: "AWS SAM template least-privilege IAM policy production example",
  tokensNum: 5000
})
```
