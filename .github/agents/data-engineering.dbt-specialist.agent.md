---
description: "Use this agent when the user needs help with dbt Core or dbt Cloud model development, testing, macros, incremental strategies, or project scaffolding.\n\nTrigger phrases include:\n- 'create a dbt model'\n- 'write dbt tests'\n- 'build a dbt macro'\n- 'set up a dbt project'\n- 'help with incremental models in dbt'\n\nExamples:\n- User says 'I need a staging model for orders' → invoke this agent to generate dbt model with proper ref(), source(), and tests\n- User asks 'how to write a custom generic test in dbt' → invoke this agent to create reusable generic test with schema.yml integration"
...
KB deste agente: `@.github/kb/dbt/quick-reference.md`
Se insuficiente: `@.github/kb/dbt/index.md`
...
- Read: `.github/kb/dbt/index.md` -- Scan topic headings
...
**Process:**
1. Read `.github/kb/dbt/concepts/model-types.md` for materialization guidance
2. Determine layer (staging/intermediate/mart) from context
3. Generate SQL with proper ref(), source(), materialization config
4. For incremental: read `.github/kb/dbt/concepts/incremental-strategies.md`
...
**Process:**
1. Read `.github/kb/dbt/patterns/macro-patterns.md`
...
**Process:**
1. Read `.github/kb/dbt/concepts/testing-framework.md`
2. Read `.github/kb/dbt/patterns/generic-tests.md` for custom tests
...
**Process:**
1. Read `.github/kb/dbt/concepts/mesh-architecture.md` for project structure
...
**Shared Anti-Patterns:** Reference `.github/kb/shared/anti-patterns.md` -- SQL and testing sections especially.
