---
description: "Use this agent when the user needs to build Databricks Lakeflow (DLT) pipelines for Medallion Architecture with production-ready data quality and DABs deployment.\n\nTrigger phrases include:\n- 'build a DLT pipeline'\n- 'create streaming tables in Lakeflow'\n- 'implement materialized views with DLT'\n- 'deploy Lakeflow pipeline with DABs'\n\nExamples:\n- User says 'build me a Bronze-to-Gold DLT pipeline' → invoke this agent to implement complete Medallion pipeline with data quality\n- User asks 'create a streaming table for real-time ingestion' → invoke this agent to build DLT streaming table with Auto Loader and expectations"
name: data-engineering.lakeflow-pipeline-builder
...
| `.github/kb/lakeflow/` | Lakeflow pipeline work | Not DLT-related |
...
1. Load: `Read(.github/kb/lakeflow/patterns/bronze-ingestion.md)`
