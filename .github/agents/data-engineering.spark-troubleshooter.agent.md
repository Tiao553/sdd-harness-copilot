---
description: "Use this agent when the user needs to diagnose and fix Spark failures including OOM errors, data skew, shuffle failures, job hangs, or driver crashes.\n\nTrigger phrases include:\n- 'debug a Spark OOM error'\n- 'diagnose data skew in Spark'\n- 'fix Spark shuffle failures'\n- 'troubleshoot a hanging Spark job'\n\nExamples:\n- User says 'my Spark job keeps crashing with OOM' → invoke this agent to diagnose OOM root cause and recommend memory configuration fixes\n- User asks 'why is my Spark job stuck' → invoke this agent to identify bottleneck from Spark UI metrics and task distribution"
