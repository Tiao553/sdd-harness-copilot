...
> **Spec-Driven Development Framework for Data Engineering on GitHub Copilot**
>
...
| **Solution** | Validated SDD workflow with 62 specialized AI agents, 26 KB domains, and command skills |
| **Target Audience** | Data engineering teams using GitHub Copilot |
| **License** | MIT |

### What This Is

AgentSpec transforms requirements into working code with full traceability. It provides a structured validated SDD development workflow (Brainstorm -> Define -> Design -> Build -> Validate -> Ship) powered by specialized AI agents that match to tasks automatically, grounded by curated knowledge bases for accuracy.
...
3. **Strength:** Mandatory Validate gate prevents unvalidated Build output from reaching Ship
...
|---|----------|-----------|--------|
| D1 | SDD pipeline (Brainstorm->Define->Design->Build->Validate->Ship) | Balances rigor with pragmatism and blocks unvalidated shipping | **Implemented** |
...
| D4 | Mandatory Validate gate after Build | Build proves creation; Validate proves spec alignment and ship readiness | **Implemented** |
...
| # | Decision | Rationale | Status |
|---|----------|-----------|--------|
| D10 | Validate gate after Build | Catch implementation drift before Ship | **Implemented** |
...
| 3.5 | `/validate` | Sonnet | DEFINE + DESIGN + BUILD_REPORT + code | VALIDATION_REPORT + RUNBOOK/ROADMAP | Score >= 90 and zero CRITICAL for Ship |
| 4 | `/ship` | Haiku | Approved validation artifacts | archive/ + SHIPPED_*.md | Lessons captured |
...
62 agent files              role: "Data
                              Transformation
                              Engineer"
```

**Framework-Agnostic:** New agents added to `.github/agents/` automatically become available for matching -- zero configuration.
...
AgentSpec leverages an ecosystem of **62 specialized agents** across 8 categories:
...
Domains are registered in `.github/kb/_index.yaml`. Use `/create-kb` to add new domains.
...
|----|-------------|----------|--------|
| FR-001 | System must provide validated SDD SDD workflow | P0-Critical | Implemented |
...
|----|------------|------|--------|
| C-001 | Must work with GitHub Copilot CLI | Technical | Required |
...
| Code ownership | Unclear | Agent attribution (62 agents) |
...
```bash
# Location: .github/agents/{category}.{agent-name}.agent.md
touch .github/agents/data-engineering.iceberg-specialist.agent.md
```
...
3. **The agent is automatically discoverable** -- Design phase will find it via `Glob(.github/agents/*.agent.md)`
...
```bash
mkdir -p .github/kb/iceberg/{concepts,patterns}
touch .github/kb/iceberg/{index.md,quick-reference.md}
```

3. **Register in KB index** (`.github/kb/_index.yaml`):

```yaml
domains:
  iceberg:
    description: "Apache Iceberg table format patterns"
    entry_point: ".github/kb/iceberg/index.md"
```
...
/workflow-commands /define .github/sdd/features/BRAINSTORM_ORDERS_PIPELINE.md

# Phase 2: Design with Agent Matching
/workflow-commands /design .github/sdd/features/DEFINE_ORDERS_PIPELINE.md

# Phase 3: Build with Agent Delegation
/workflow-commands /build .github/sdd/features/DESIGN_ORDERS_PIPELINE.md
...
# Phase 4: Archive after validation approval
/workflow-commands /ship .github/sdd/features/DEFINE_ORDERS_PIPELINE.md
```
...
| Resource | Location |
|----------|----------|
| SDD Index | `.github/sdd/_index.md` |
| Architecture | `.github/sdd/architecture/ARCHITECTURE.md` |
| Workflow Contracts | `.github/sdd/architecture/WORKFLOW_CONTRACTS.yaml` |
| Templates | `.github/sdd/templates/` |
| Archive | `.github/sdd/archive/` |
| Agents (62 + DEFAULT) | `.github/agents/` |
| Knowledge Base (26) | `.github/kb/` |
| Workflow skill | `.github/skills/workflow-commands/SKILL.md` |
| Validate skill | `.github/skills/validate/SKILL.md` |
| Data engineering skill | `.github/skills/data-engineering-commands/SKILL.md` |
| Core skill | `.github/skills/core-commands/SKILL.md` |
...
|---------|------|---------|
| 2.1.0 | 2026-03-26 | Multi-cloud coverage: 62 agents across 8 categories, 26 KB domains |
