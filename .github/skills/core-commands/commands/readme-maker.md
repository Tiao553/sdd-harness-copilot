...
/sync-context                    # Update .github/copilot-instructions.md
...
|---|---|---|
| Input check | Projeto contém pelo menos 1 arquivo de configuração (package.json, pyproject.toml, etc.) | Alerte e gere README mínimo |
| Content check | README gerado não contém placeholders (`TODO`, `{}`, `{project}`) | Remova placeholders antes de salvar |
| Commands check | Todos os comandos de instalação e uso foram testados e funcionam | Marque comandos não verificados com aviso |
| Links check | Todos os links internos apontam para arquivos existentes | Remova ou corrija links quebrados |
| Output check | README.md salvo no diretório correto com formatação válida | Reporte erro |
