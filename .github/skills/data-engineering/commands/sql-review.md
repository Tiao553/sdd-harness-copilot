...
|---|---|---|
| Input check | Código SQL ou modelo dbt fornecido contém pelo menos 1 query válida | Pare e reporte |
| Agent check | `code-reviewer` agent carregado e KB `sql-patterns` disponível | Pare e reporte |
| Review check | Relatório cobre anti-patterns, PII, performance e cobertura de testes | Alerte sobre categorias não avaliadas |
| Output check | Relatório de review gerado com severidade classificada e sugestões de correção | Reporte erro |
